...
int load_from_csv(void *ds, DSInterface iface, const char *filename) {
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (!token) continue; strncpy(r.student_id, token, 12);
		
		token = strtok(NULL, ",");
		if (!token) continue; strncpy(r.name, token, 19);
		
		token = strtok(NULL, ",");
		if (!token) continue; strncpy(r.college, token, 49);
		
		token = strtok(NULL, ",");
		if (!token) continue; strncpy(r.course_id, token, 8);
		
		token = strtok(NULL, ",");
		if (!token) continue; strncpy(r.course_name, token, 49);
		
		token = strtok(NULL, ",");
		if (!token) continue; r.credit = atof(token);
		
		token = strtok(NULL, ",");
		if (!token) continue; strncpy(r.semester, token, 7);
		
		token = strtok(NULL, ",");
		if (!token) continue; strncpy(r.enroll_date, token, 10);
		
		token = strtok(NULL, ",");
		if (!token) continue; r.score = atoi(token);
